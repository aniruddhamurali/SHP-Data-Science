...
gut_rich <- data.frame(mag = seq(from = 3,to = 7,by = .5),
                       count = sapply(seq(3,7,.5),count))

library("ggplot2")
ggplot(gut_rich) +
  aes(x = mag, y = log(count, base = 10)) +
  geom_point() +
  theme_bw()
...
ggplot(gut_rich) +
  ...
  geom_smooth(method="lm", color = "orange", se = FALSE, formula = sin(x) + cos(x))

fit <- lm(log(count,base=10) ~ mag, gut_rich)
fit
summary(fit)
predict(fit, newdata = data.frame(mag=0))
